...
class MOSI:
    def __init__(self, config):
        ...
        if config.sdk_dir is None:
            ...
            sys.path.append(config.sdk_dir)
        
        DATA_PATH = '/mnt/soyeon/workspace/HCAI-Proj/data/MOSI'
        ...
        try:
            ...
            # load pickle file
            pickle_filename = './data/MOSI/mosi_data_noalign.pkl'
            csv_filename = './data/MOSI/MOSI-label.csv'
            ...
            if True:
                v = np.concatenate((train_split['vision'],dev_split['vision'], test_split['vision']),axis=0)
                vlens = get_length(v)

                a = np.concatenate((train_split['audio'],dev_split['audio'], test_split['audio']),axis=0)
                alens = get_length(a)
                
                label = np.concatenate((train_split['labels'],dev_split['labels'], test_split['labels']),axis=0)

                L_V = v.shape[1]
                L_A = a.shape[1]

            all_id = np.concatenate((train_split['id'], dev_split['id'], test_split['id']),axis=0)[:,0]
            all_id_list = list(map(lambda x: x.decode('utf-8'), all_id.tolist()))

            train_size = len(train_split['id'])
            dev_size = len(dev_split['id'])
            test_size = len(test_split['id'])
...
class MOSEI:
    def __init__(self, config):
        ...
        DATA_PATH = '/mnt/soyeon/workspace/HCAI-Proj/data/MOSEI'
        CACHE_PATH = DATA_PATH + '/embedding_and_mapping.pt'
        ...
        try:
            ...
            pickle_filename = './data/MOSEI/mosei_senti_data_noalign.pkl'
            csv_filename = './data/MOSEI/MOSEI-label.csv'
